...
function getPolynom(...args) {
  return (arg) => args
    .reduce((sum, item, index) => sum + item * (arg ** (args.length - index - 1)), 0);
}
...
function retry(func, attempts) {
  return () => {
    try {
      ...
      if (attempts) return retry(func, attempts - 1)();
    }
    return '';
  };
}
...
function logger(func, logFunc) {
  return (...args) => {
    const argsValus = JSON.stringify(Array.from(args)).slice(1, -1);
    logFunc(`${func.name}(${argsValus}) starts`);
    const output = func(...args);
    logFunc(`${func.name}(${argsValus}) ends`);
    return output;
  };
}
